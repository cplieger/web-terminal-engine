...
> **Note on DECRQSS:** unlike the other DCS sequences above, **DECRQSS (`DCS $ q … ST`, Request Status String) is supported.** The emulator answers SGR (`m`), DECSTBM scroll region (`r`), and DECSCUSR cursor style (` q`) queries with a valid `DCS 1 $ r … ST` reply and returns `DCS 0 $ r ST` for unrecognized selectors (see `vt/dcs.go`).
