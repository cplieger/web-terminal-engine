...
- **`connectStatusStream`** and the `SessionInfo` / `SessionStatus` / `StatusStream` types: The SSE client for `/api/sessions/events` that drives per-tab status. `SessionInfo` carries the resolved `title`, the user's `pinnedTitle`, `order` (the session's position in the display order the server owns, which is what a consumer sorts its tabs by; absent from a pre-3.10.0 server, and from a status event marking a session removed), and the secondary `activity` + `activityCount` pair, typed as a plain `string` and `number` so a value from a newer server needs no cast. `LineStore` and `CONTROL_FRAME_PREFIX` are also exported, as are the `WS_PATH` / `SESSIONS_PATH` / `SESSION_EVENTS_PATH` route constants mirroring the Go `terminal` package's mount contract.
