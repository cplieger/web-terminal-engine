// All-codes display conformance in a real headless Chromium: the engine's real
// wire frame for EVERY SGR display code is rendered and each cell's
// getComputedStyle is dumped over CDP and asserted against the SPEC. The fixture
// pair comes from terminal/render_golden_test.go TestRenderGoldenAllCodes:
// all-codes.bin is the frame, all-codes.manifest.json is which SGR each row
// carries ("what was fed", never the expected output). Expectations derive from
// the published standards (spec-colors.ts, the ANSI/CSS attribute mapping), not
// from render.ts, so a mismatch is a finding. Run with `npm run test:e2e`.
...
test.describe("all-codes display conformance (engine wire → real chromium → computed style)", () => {
  test("every SGR display code renders to its spec computed style", async ({ page }) => {
    ...
    // Decode the engine's real frame and render it with the production renderer.
    ...
    // expect.soft so a single run reports EVERY deviation across all 300+ codes.
    ...
    // Basic-16 palette. SPEC: the engine's default palette is kitty's published
    // default (spec-colors.ts `kitty16`, derived independently of wire.go), so
    // each slot is pinned to its exact RGB (fg 30-37 / 90-97, bg 40-47 / 100-107);
    // a consistency-only check passes 16 distinct-but-wrong colors. Then the ANSI
    // consistency: SGR 38;5;n (n<16) addresses the SAME slot as 3n/9n.
    for (let n = 0; n < 8; n++) {
      expect
