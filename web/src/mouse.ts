// Mouse event encoding for terminal input, SGR 1006 only (no X10, urxvt or raw
// encoding): ESC[<code;col;rowM for press/move, ESC[<code;col;rowm for release,
// 1-based coordinates, xterm button codes (0/1/2 buttons, 64/65 wheel, +4
// shift, +8 alt, +16 ctrl, +32 motion). Reports are BEST-EFFORT: one carries no
// sequence number and describes a screen a resume may since have repainted, so
// a refused report is forgotten rather than retried. Focus is transport state
// the server derives and lives in `connection.setClientFocus`.
...
/**
 * Encode a mouse event into an SGR 1006 escape sequence: `code` is the xterm
 * button code plus modifier bits, `col` and `row` are 1-based, and `release`
 * selects the `m` terminator over `M`.
 */
...
/**
 * Adapter the consumer supplies so the mouse controller can: send encoded
...
/** `MouseInputHandler` plus the mode state the listeners gate on. */
export interface MouseControllerOptions extends MouseInputHandler {
  modes: Pick<ModeState, "getMouseMode" | "isMouseSGR" | "isMousePixels">;
}

/** Pointer and wheel forwarding for one terminal element. */
export interface MouseController {
  /**
   * Cancels a gesture left in flight once the resume has declared the server's
   * capabilities: emits one release for each recorded button the last reading
   * says is now up, so a drag is not left pressed across a reconnect.
   */
  resyncGesture(): void;
  /** Forgets the held gesture without reporting anything, for a consumer pointing this terminal at another session. */
  disarmGesture(): void;
  /** Detaches the four listeners, cancels the pending motion frame and forgets the gesture. Idempotent. */
  dispose(): void;
}

/**
 * Creates a mouse controller over `opts.termElement()`. The listeners gate on
 * the active mouse mode and SGR 1006 encoding, so they are inert until the
 * application enables tracking. Throws, holding nothing, when a listener cannot
 * be attached.
 */
export function createMouseController(opts: MouseControllerOptions): MouseController {
  // Invariant: `modes` and `handler` are read by member call at each call site, never as captured methods.
  const modes = opts.modes;
  let handler: MouseInputHandler | null = opts;
  ...
  let lastMotion = "";
  // The element the listeners are attached to, so dispose detaches the exact set.
  ...
  function trackingActive(): boolean {
    return modes.getMouseMode() !== 0 && (modes.isMouseSGR() || modes.isMousePixels());
  }
  ...
  // Called on the resumeAck, not the socket's open: the ephemeral channel is
  // not yet known at open, so a release sent there would take the reliable
  // fallback and become the one replayable mouse report. Reports PRESENT state
  // (what RFB and Guacamole do implicitly with their next message), one release
  // per recorded button the last reading says is UP; a release that happened
  // off-element is invisible either way.
  function resyncGesture(): void {
    if (!handler) {
      return;
    }
    cancelPendingMotion();
  ...
  // The record says what the OUTGOING session's application believes is held,
  // and sessions multiplex over one controller, so a release surviving the
  // switch would reach a session that never saw the press.
  ...
  function onWheel(e: WheelEvent): void {
    ...
    cancelPendingMotion();
    // A gesture with NO vertical component (a horizontal swipe, shift-wheel
    // routed to deltaX) has no wheel button: xterm.js reports nothing and still
    // calls preventDefault, since a terminal owns the wheel and a sideways swipe
    // let through would scroll the page. A wheel report is never coalesced: it
    // is a NOTCH COUNT against a screen, so dropping one loses scroll distance.
    if (e.deltaY !== 0) {
  ...
  return { resyncGesture, disarmGesture, dispose };
}
